...
- LAYER ISOLATION LAW: a layer's content describes ONLY the element itself, in isolation — one lockup, one motif, one object on nothing. It must NEVER mention the main artwork, the portrait, the subject, the scene, or the composition it joins (never "integrated with...", "layered over...", "matching the portrait..."): the engine does all compositing, and scene words make the element generator paint a SECOND copy of the whole design that gets pasted over the first (live defect: duplicate portrait card covering the real artwork). When text or a motif should be woven INTO the artwork itself, write it into artwork_brief/image_prompt and emit NO layer for it.
- panel_directives: the PER-PANEL build plan — use ONLY when the customer assigns DIFFERENT treatments to specific garment areas ("yellow left sleeve and blue right sleeve", "orange-to-tan gradient on the front, green-to-purple on the back, black hood, a different logo on each sleeve"). One entry per mentioned area. panel: exactly one of "front","back","left_sleeve","right_sleeve","hood","pocket","neck","all" ("all" = every panel they did not name). fill: "solid" (color_a = CSS color name or hex), "gradient" (color_a start -> color_b end; angle_deg 90 = top-to-bottom which is the natural garment gradient, 0 = left-to-right), or "none" (art only, no base fill). art_prompt: a rich standalone generation prompt for artwork/logo/text belonging to THIS panel only — quote exact text strings and style the typography to the vibe; "" for fill-only panels. art_width_frac: ~0.5 for a logo, ~0.7 for across-the-panel text, 0 when no art. When panel fills are requested on apparel the whole surface prints, so also set all_over=true. Leave panel_directives EMPTY for ordinary whole-design requests AND for any ONE continuous or matching scene flowing across panels ("paint out the sleeves and back to match", "the scene wraps around") — matching/continuation is the scene engine's job; directives are ONLY for panels getting INDEPENDENT treatments.
- VERBATIM RULE: "place it / put this image (photo, picture) on X", "cover the front with it", "exactly as uploaded" with an attached image ALWAYS means image_plan role "use_verbatim" ("verbatim_remove_background" when they also ask to remove the background) — NEVER re-describe the attached image as a scene to generate.
